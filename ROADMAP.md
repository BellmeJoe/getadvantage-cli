...
| **0.14.0** | **Minor** — new user-facing `getadvantage feedback` command (print-only GitHub issue URL; nothing sent, no browser, zero network, always exit 0, not a gate) plus nine-lane train: ops retained-team detector + `incomplete_results` guard, Marketplace listing readiness, README/version-claim currency, redaction catalogue unification + narrowing repair. No new check. Never advertise parked RLS | **REVIEW_PENDING** (version bumped in-tree; product tip `3afc50b` + feedback `8eb494b` + redaction `ec58b49`/`53339ff`/`f19797b`; not tagged / not published; needs release-scoped independent audit; rollback target after publish: `0.13.1` / `v0.13.1` / `e5b06f3`) |
