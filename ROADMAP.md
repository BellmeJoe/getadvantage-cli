...
| **0.11.1** | Paste-ready remediation on **secret-scan NO-GO only**: each blocking secret finding names a smallest safe next edit + pattern-aware `secrets.ignore` snippet (auth-id hashes for unique patterns; paths-only for non-unique). Emission-only — no new check, no verdict change, no exit-code change | **Released** (npm + tag v0.11.1 + floating v1, 2026-07-31; product `25f0e54`; release `66ffb9b`; cold published verified; rollback `0.11.0` / `v0.11.0` / `96fb5f8`) |
| **0.12.0** | Invisible mode (B2): `init --claude-code` installs automatic Claude Code gate hooks + in-repo proof receipt (zero telemetry); agent-trigger profile omits Dirty-tree on hooks only; Cursor = detect-and-refuse | **Released** (npm + tag v0.12.0 + floating v1, 2026-08-03; product `fe9e2ad`; release `b8be10c`; cold published GO ~5.3s; activation ~11.3s; rollback `0.11.1` / `v0.11.1` / `66ffb9b`; *scope verified; semantic correctness not proven*) |
| **0.12.1** | Presentation density train: symlink-escape `120000` regression coverage (`5f0422d`) + remediation output density (`9b65aec`) + API-map density (`4b69b51`). Patch only — no new check, no verdict/exit/`--json`/SARIF change. Measured: CLI `check` 109→56 lines; site `check` 143→82 lines | **Released** (npm + tag v0.12.1 + floating v1, 2026-08-05; product `4b69b51`; release `e2f8665`; cold published GO ~5.0s; rollback `0.11.1` / `v0.11.1` / `66ffb9b` at train time; never advertise RLS) |
| **0.12.2** | Action PR / job summary findings density: every finding (not only the first) with `file:line` + paste-ready remediation, blocking-first (bounded caps). Presentation only | **Released** (npm + tag v0.12.2 + floating v1, 2026-08-07; product `653a693`; release `1e451ac`; suite 226/226; cold published GO ~5.0s; rollback `0.12.1` / `v0.12.1` / `e2f8665`) |
| **0.13.0** | `--report-dry-run` transparency mode: exact `buildRunBody()` preview, endpoint + byte size + key-resolved-source (never value), **zero network**. Minor — additive flag. Transparency gap closed, not a leak | **Released** (npm + tag v0.13.0 + floating v1, 2026-08-13; product `cdb923c` (feat `472ca83` + test rescope `cdb923c`); release `bdc8b04`; suite 230/230; cold published GO ~38.6s; rollback `0.12.2` / `v0.12.2` / `1e451ac`) |
| **0.13.1** | Presentation + correctness train on the 0.13.0 surface: collapsed secret-row `file:line` restore (`ce5c182`) + dry-run summary density + body/transport key banner + paste-ready `secrets.ignore` JSON repair (`ec92722`). Patch only — no new check, no verdict/exit/`--json`/SARIF change | **Released** (npm + tag v0.13.1 + floating v1, 2026-08-14; products `ce5c182` + `ec92722`; release `e5b06f3`; suite 234/234; cold published GO ~17.2s; rollback `0.12.2` / `v0.12.2` / `1e451ac`) |
| **0.14.0** | **Minor** — new user-facing `getadvantage feedback` command (print-only GitHub issue URL; nothing sent, no browser, zero network, always exit 0, not a gate) plus nine-lane train: ops retained-team detector + `incomplete_results` guard, Marketplace listing readiness, README/version-claim currency, redaction catalogue unification + narrowing repair. No new check. Never advertise parked RLS | **Released** (npm + tag v0.14.0 + floating v1, 2026-08-20; release `cc9d39a`; rollback `0.13.1` / `v0.13.1` / `e5b06f3`) |
| **0.14.1** | **Patch** — first-contact classification (bare / unreadable `.git` / non-git: human guidance on stdout, 0 B stderr) + packed-tarball hygiene (`tests/` no longer ships). No new command, check, flag, or capability. Policy gate **not** in this train | **Released** (npm + tag v0.14.1 + floating v1, 2026-08-23; release `f098c5e`; suite 318/318; evidence 8/8; rollback `0.14.0` / `v0.14.0` / `cc9d39a`) |
| Later | Request-time **policy gate** (`getadvantage gate`) — deterministic outbound stdin filter (BLOCK / `--redact`), PII + denylist, local proof record. Not inbound. Not live as a request interceptor until a release train says so. | Roadmap |
