...
```
✗ Secret scan — 1 possible secret in committed/staged files — remove + rotate before shipping.
    .env → Stripe live secret key: sk_live_…0000 (32 chars)
    Smallest safe next edit — Stripe live secret key at .env:2: remove, rotate, re-run check.

✗ Tracked .env file — 1 .env file tracked by git — a committed .env is a leak by itself, whatever it contains.
    .env
    Remove it from git (git rm --cached <file>), add it to .gitignore, and ROTATE every key that file ever held.
