...
> writing a local per-request proof record under `.getadvantage/gate-proofs/`.
> **Say what it is not:** not inbound, not a proxy or daemon, not a classifier, not
> automatic — you pipe into it explicitly. Base64 evasion decodes **one level only**;
> `--redact` re-encodes UTF-8, so non-UTF-8 input may carry U+FFFD. The secret
> catalogue was **moved, not rewritten**: 20 pattern ids and 20 regex bodies are
> byte-identical to `0.14.2`, so `check` verdicts are unchanged.
>
> **What 0.14.2 shipped (prior patch, still true):** before 0.14.2 a
...
| **Invisible mode (Claude Code)** | One command installs the gate as an **automatic hook** so every agent session is gated by default; writes in-repo proof receipt with **zero telemetry**. Cursor is **detect-and-refuse** (not automatic). | `npx getadvantage init --claude-code` · status/uninstall/bypass · receipt `.getadvantage/INVISIBLE-MODE.md` | product `fe9e2ad` · cold published activation on `0.15.3`, isolated npm cache: `init --claude-code` **exit 0, 3.54s**, then first gate **exit 0, 4.21s** — **cold start to installed hook and first gate 7.75s** (measured 2026-09-10 09:4x Amman by `grok-build-product-cycle`, `runs/2026-09-10-grok-build-CYCLE-0853.md` §9) · *scope verified; semantic correctness not proven* |
