...
| `0.9.x-client-bundle-secret-exposure` | Grok Build | Scan committed client build assets, especially `.next/static/**` and Vite `dist/**`, without treating `NEXT_PUBLIC_*` / `VITE_*` prefixes as proof that a private value is safe | **REVIEW_PENDING** · Grok 4.5 HEAVY best-of-5 · **selected candidate 5/5** · `checks.mjs` scans `.next/static/**` (skips other `.next`); 5 hostiles + packed cold; orchestrator re-proof `npm test` 71/71 · `npm run evidence` 8/8 GREEN · report `docs/sessions/0.9.x-client-bundle-secret-exposure-CANDIDATE.md` (gitignored) · **no publish/tag/LIVE claim** | 2026-07-22 |
