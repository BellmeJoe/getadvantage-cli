...
| `0.9.x-client-bundle-secret-exposure` | Grok Build | Scan committed client build assets, especially `.next/static/**` and Vite `dist/**`, without treating `NEXT_PUBLIC_*` / `VITE_*` prefixes as proof that a private value is safe | **QUEUED_NEXT** · no worker and no candidate yet · dispatch next scheduled cycle using `C:\Users\ben\agent-ops\GROK-NEXT-TASK.md` · cold acceptance: `.next/static` and `dist` private key NO-GO, public client config not a secret NO-GO, existing 8/8 evidence preserved · hostile fixtures: skipped `.next` key, `VITE_`-prefixed live key, public anon config | 2026-07-21 |
