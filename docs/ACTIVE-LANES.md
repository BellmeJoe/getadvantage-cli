...
| `0.9.0-first-party-action-pr-summary` | Grok Build | root `action.yml`, `action/{main,summary,enforce,install}.mjs`, scrubbed install, eligible-skip red, token ownership, SARIF nonce, `uses: ./` publish gate, published-source identity (npm gitHead), annotated tag peel, hostile tests | **TECHNICALLY_BLOCKED** · YAML repair committed `8779a7f` · Actions [29841391238](https://github.com/BellmeJoe/getadvantage-cli/actions/runs/29841391238) parse OK + tests/evidence GREEN · failed real `uses: ./` gate: gate NO-GO on fixture secrets in `tests/run.mjs` · npm publish skipped · live npm **0.8.4** · social pack 0.8.4 LIVE · no v0.9.0/v1. Next: fix self-gate working tree (exclude/allowlist fixtures or clean fixture cwd) — new independent audit required | 2026-07-21 |
