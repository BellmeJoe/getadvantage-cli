...
function detectDeploy(cwd) {
  ...
  if (existsSync(path.join(cwd, "cli", "ship-safe", "deploy.mjs"))) {
    notes.push("getAdvantage safe-deploy ritual (`npx getadvantage deploy`) — clean detached worktree + host-prefix confirm.");
  }
...
function renderBrief(cwd, notes) {
  ...
  L.push("generator: getadvantage brief");
  L.push("---");
  L.push("");
  L.push(BANNER);
  L.push("");
  ...
  L.push("");
  L.push("*Generated by the same scanners `npx getadvantage check` uses — so this map and");
