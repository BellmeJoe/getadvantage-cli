...
| Supabase RLS / ungated mutations | **BLOCKS** (when proven) · **not-yet-live** (shipped in-tree; npm still 0.10.1) | Static **policy-state** over tracked `supabase/migrations/**/*.sql` (ordered by path name) and high-bar `supabase/functions/**` sources. **NO-GO** only on proven final-state public write risk: RLS disabled on a `public` table, or `CREATE POLICY` with `USING (true)` / `WITH CHECK (true)` on INSERT/UPDATE/DELETE/ALL for client-facing roles. Emits exact file:line, why it matters, and a **paste-ready smallest safe next edit**. Incomplete/dynamic/malformed SQL or Edge wrappers → **warn** (not checkable ≠ GO). No Supabase migration/function evidence → honest **skip**. Comments, strings, and test/fixture paths are ignored. Service-role Edge paths are **not** called public. Does **not** execute SQL, call Supabase, or prove end-to-end authorization. |
