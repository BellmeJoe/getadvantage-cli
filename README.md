...
| `getadvantage intent` | **Intent Contract** (local change-scope proof). `intent init --goal "…" --allow <glob> [--deny …]` writes `.getadvantage/intent.json`. **Commit it before the agent starts** — authorization is the committed `HEAD` blob, not a worktree copy the agent could broaden. After the agent works, `intent check` compares staged + unstaged + deleted + renamed + untracked paths → **GO / NO-GO** + a stable proof receipt (goal, contract hash, baseline, changed paths, violations). `check` includes this result when a trusted contract is present; projects without a contract keep existing checks only (no false “intent verified”). **Limitation:** *scope verified; semantic correctness not proven.* |
...
# 3. Prove the working tree stayed in scope
npx getadvantage intent check
# → GO or NO-GO + receipt (contract hash, changed paths, violations)
#    scope verified; semantic correctness not proven
```

Deny overrides allow. Renames check both old and new paths. Editing the worktree contract to widen `--allow` cannot authorize itself. Absolute paths, traversal, malformed schema, and remote/PR base refs fail closed.
...
| Intent Contract | **BLOCKS** (when present) | When a **committed** `.getadvantage/intent.json` exists on `HEAD`, every staged/unstaged/deleted/renamed/untracked path is checked against allow/deny/require/maxFiles. No contract → check omitted (never a false “intent verified”). Scope only — not semantic correctness. |
