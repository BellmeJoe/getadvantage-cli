...
| `getadvantage map` | A read-only map of what your app has: project estate (including **Vite / React / Supabase client orientation** — evidence-only statuses and paths; **LIVE**), API surface (what is gated, what mutates — with a `⚠` on any mutating route that has no obvious gate), agents & integrations, schedules & jobs. Route mapping covers **Next.js** App Router, **Express/Fastify**, and **Flask/FastAPI** (best-effort regex parsing — no code is run); client SPAs get an honest “route mapping does not apply” line — never invented Express routes. Supabase SDK detection is not an RLS/auth verdict. Orientation, never a verdict — always exits `0`. |
...
A nested value that matches a listed secret pattern is refused, and the match
is not copied into the error, including the `--json` error document. A name
that matches a listed secret pattern, passed as an MCP tool, method, or
property name, is not copied into the JSON-RPC error message; a name that does
not match is still named, so the agent can see what failed. Successful MCP
reads still return repo content, which may include secrets the owner asked to
read. This name scrub is not `check`: `check` uses lookbehind anchors and
validators; a prefixed shape can miss there and still be withheld here. An
approval file that shares storage with another name is not written. Both commands are in this
...
shipped action-compatible release). Prefer `@v1` for the current major. For
bit-for-bit reproducibility, pin `uses: BellmeJoe/getadvantage-cli@<tag>` to an
exact release tag from https://github.com/BellmeJoe/getadvantage-cli/releases
(do not hand-list tags here — they go stale). npm installs use package versions
/ dist-tags, not the floating `v1` git tag.
