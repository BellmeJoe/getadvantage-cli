...
| `getadvantage gate` | **Policy gate** — outbound stdin filter (pipe a prompt in, BLOCK by default or `--redact` to mask). Same secret corpus as `check`, plus gate-only PII and an optional project denylist in `.getadvantage/config.json`. Writes a local proof record under `.getadvantage/gate-proofs/` (hashes only, never the raw value). Stdin bounds: max 1,048,576 bytes (1 MiB), idle 30 s, total 120 s — hitting one fails closed (non-zero exit, nothing of the payload on stdout, named stderr reason, proof action `INCOMPLETE`, never `PASS`). Base64 evasion decodes one level; a twice-encoded secret is not detected. PASS writes the raw stdin bytes; `--redact` re-encodes UTF-8, so non-UTF-8 input may contain U+FFFD replacement characters. Not inbound, not a proxy, not a daemon. **Not in published `getadvantage@0.14.1`.** `npm i getadvantage` / `npx getadvantage` today does not include this command. Not live as a request interceptor until a release train says so. |
